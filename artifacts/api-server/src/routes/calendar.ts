...
router.post("/calendar/advance", async (req, res) => {
  ...
  // Non-regional slots: move the rest of the World Tour first, then pause if
  // the player's team has a match today.
  if (todayRounds.some(r => !isRegionalSlot(r))) {
    const wtRounds = todayRounds.filter(r => !isRegionalSlot(r));
    const maxAIRound = Math.max(...wtRounds);

    // R-29: the rest of the World Tour moves with the calendar. The draw is
    // made on the first World Tour day, every AI fixture due by today is played
    // through the player's own engine, and the finals are seeded when their
    // round arrives. This replaced autoSimulateAIMatches: a 55% coin flip over
    // every OTHER team's scheduled matches in the database — other careers'
    // fixtures included — writing their teams' wins and losses.
    //
    // Deliberately not wrapped in a bare catch: that is how the coin flip's
    // cross-career writes stayed invisible. A World Tour that cannot move is a
    // fault and should say so.
    const wt = await advanceWorldTour({
      careerSaveId:         requireCareerSaveId(req.activeCareerSaveId),
      seasonYear:           season.year,
      playerTeamId:         team.id,
      upToRound:            maxAIRound,
      allowRegionalCatchUp: true,
    });
    if (wt.blocked) req.log.warn({ round: maxAIRound, reason: wt.blocked }, "World Tour could not advance");
    if (wt.aiPlayed > 0) {
      events.push(`${wt.aiPlayed} World Tour match${wt.aiPlayed === 1 ? "" : "es"} played across the field`);
    }
...
// Move past match day once the pending match has actually been played (R-29)
...
router.post("/calendar/skip-match", async (req, res) => {
  ...
  // R-29: this used to write a random 2-1 or 1-2 straight onto the match — no
  // engine, no ranking points, no opponent credited — a result nobody played.
  // "Skip for now (auto-sim in background)" now means exactly that: the client
  // simulates through POST /matches/:id/simulate, the same engine as every
  // other result, then calls this to move the day on.
  const matchRows = await db.select().from(matchesTable).where(eq(matchesTable.id, calendar.pendingMatchId)).limit(1);
  const match = matchRows[0];
  if (match && match.status !== "completed" && match.status !== NOT_QUALIFIED) {
    res.status(409).json({
      error: "The pending match has not been played. Simulate it first — skipping no longer invents a result.",
      pendingMatchId: calendar.pendingMatchId,
    });
    return;
  }
...
router.get("/calendar/annual", async (req, res) => {
  ...
  type EvType = "regional" | "world_tour" | "finals" | "holiday" | "financial" | "contract" | "facility" | "olympic_qualifier" | "olympic";
  const events: {
