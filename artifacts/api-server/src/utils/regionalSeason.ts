 ...
 */
/**
 * Simulate one regional-league fixture.
 *
 * Delegates to the shared match engine. It used to be a separate model —
 * `effectiveHome > effectiveAway` with uniform noise, sets assigned by coin
 * flip, and point totals from Math.random()*18+3 that ignored team strength
 * entirely. Measured against the shared engine at the same rating gaps it
 * diverged by up to 26.9pp and produced a GUARANTEED win at +25, which the
 * engine design explicitly rules out.
 *
 * A league table generated by different maths than the player's own results is
 * fiction, which is what invariant I7 exists to prevent.
 */
export function simulateFixtureResult(
  ...
  const p = pointProbability(homeRating, awayRating, { homeAdvantage: true });
  const match = simulateMatch(p);

  let homeMatchPoints = 0;
  let awayMatchPoints = 0;
  for (const set of match.sets) {
    homeMatchPoints += set.home;
    awayMatchPoints += set.away;
  }

  return {
    homeSets:  match.homeScore,
    awaySets:  match.awayScore,
    homeMatchPoints,
    awayMatchPoints,
    winnerId:  match.homeWon ? "home" : "away",
  };
}
