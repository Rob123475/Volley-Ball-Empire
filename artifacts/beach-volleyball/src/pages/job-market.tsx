...
const JOB_OFFERS: JobOffer[] = [
  {
    id:                 1,
    clubName:           "Rio Praia SC",
    continent:          "South America",
    country:            "Brazil",
    city:               "Rio de Janeiro",
    competition:        "FIVB World Tour",
    tier:               "Elite",
    salary:             28_000,
    transferBudget:     650_000,
    clubReputation:     88,
    expectedFinish:     "Top 4",
  ...
  {
    id:                 2,
    clubName:           "Barcelona Sand FC",
    continent:          "Europe",
    country:            "Spain",
    city:               "Barcelona",
    competition:        "European Beach Tour",
    tier:               "Elite",
    salary:             24_000,
  ...
  {
    id:                 3,
    clubName:           "Miami Blazers",
    continent:          "North America",
    country:            "USA",
    city:               "Miami",
    competition:        "NORCECA Beach Tour",
    tier:               "Premier",
    salary:             22_000,
    transferBudget:     420_000,
    clubReputation:     78,
    expectedFinish:     "Top 8",
  ...
  {
    id:                 4,
    clubName:           "Shanghai Stormers",
    continent:          "Asia",
    country:            "China",
    city:               "Shanghai",
    competition:        "Asian Beach Tour",
    tier:               "Premier",
    salary:             20_000,
    transferBudget:     380_000,
    clubReputation:     72,
    expectedFinish:     "Top 4",
  ...
  {
    id:                 5,
    clubName:           "Sydney Surf FC",
    continent:          "Oceania",
    country:            "Australia",
    city:               "Sydney",
    competition:        "Oceania Cup",
    tier:               "Premier",
    salary:             18_000,
  ...
  },
  {
    ...
    clubReputation:     58,
    expectedFinish:     "Reach Knockouts",
  ...
  {
    id:                 7,
    clubName:           "Cape Town Swells",
    continent:          "Africa",
    country:            "South Africa",
    city:               "Cape Town",
    competition:        "African Beach Championship",
    tier:               "Regional",
    salary:             12_000,
    transferBudget:     160_000,
    clubReputation:     54,
    expectedFinish:     "Top 8",
  ...
  },
  {
    ...
    tier:               "Regional",
    salary:             13_000,
    transferBudget:     180_000,
    clubReputation:     56,
    expectedFinish:     "Mid-table",
    requiredReputation: 30,
    squadStrength:      55,
  ...
  },
  {
    ...
    clubReputation:     44,
    expectedFinish:     "Qualify for Finals",
  ...
  {
    id:                 10,
    clubName:           "Reykjavik Frostbiters",
    continent:          "Europe",
    country:            "Iceland",
    city:               "Reykjavik",
    competition:        "European Beach Tour",
    tier:               "Emerging",
    salary:             8_000,
    transferBudget:     80_000,
    clubReputation:     38,
    expectedFinish:     "Avoid Bottom 4",
...
// ── Stat row (shared between cards and modals) ────────────────────────────────
...
function ClubLogo({ job, size = "md" }: { job: JobOffer; size?: "sm" | "md" | "lg" }) {
  const dim = size === "lg" ? "h-14 w-14 text-2xl rounded-2xl" : size === "md" ? "h-12 w-12 text-lg rounded-xl" : "h-10 w-10 text-base rounded-lg";
  return (
    <div
...
function JobCard({ job, onApply, onView }: {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/3 overflow-hidden hover:border-white/15 transition-colors flex flex-col">
      ...
      <div className="px-5 pt-4 pb-2">
        ...
        <div className="h-1.5 w-full bg-white/8 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${job.clubReputation}%`, backgroundColor: job.logoColor }}
          />
        </div>
      </div>
      ...
      <div className="flex gap-2 px-5 py-4 mt-auto">
        ...
        <Button
          ...
          style={{
            backgroundColor: `${job.logoColor}22`,
            borderColor:     `${job.logoColor}55`,
            color:            job.logoColor,
          }}
          onClick={() => onApply(job)}
        >
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-blue-500/20 bg-blue-500/8 px-4 py-3">
          ...
          <p className="text-xs text-blue-300/70 leading-relaxed">
            These are sample listings — full job offers with real club matching and negotiation will be available once the application system is live.
          </p>
        </div>
          ...
          <DialogContent className="max-w-sm border-white/10 bg-[#0f1117]">
            <DialogHeader>
              <div className="flex items-center gap-3 mb-1">
                <ClubLogo job={applyJob} size="sm" />
                <div>
                  <DialogTitle className="text-sm font-black text-white leading-tight">
                    Apply to {applyJob.clubName}
                  </DialogTitle>
                  <DialogDescription className="text-xs text-white/40 mt-0.5">
                    {applyJob.city}, {applyJob.country}
                  </DialogDescription>
                </div>
              </div>
            </DialogHeader>
            ...
            {/* Offer at-a-glance */}
            <div className="rounded-xl border border-white/8 bg-white/3 px-4">
              <StatRow icon={DollarSign} label="Salary"          value={fmtSalary(applyJob.salary)}        valueColour="text-emerald-400" />
              <StatRow icon={Wallet}     label="Transfer Budget" value={fmtBudget(applyJob.transferBudget)} valueColour="text-blue-400"    />
              <StatRow icon={TrendingUp} label="Board Expects"   value={applyJob.expectedFinish} />
            </div>
            ...
            <div className="flex items-start gap-3 rounded-xl border border-white/8 bg-white/3 p-4">
              ...
              <p className="text-sm text-white/55 leading-relaxed">
                Applications and contract negotiations are coming in a future update. Full interactive hiring will be available once the transfer system is complete.
              </p>
            </div>
