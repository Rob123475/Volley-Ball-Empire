...
**Added 21 Sep 2026:** R-80, the full pre-push check — item 20 below and `docs/PRE-PUSH-CHECK.md`.
...
| 20 | R-80 | `84837be`…`7c36cef` | A full pre-push check of the whole game, eight gates. Nine defects found and fixed, one commit each: five in the game — a dead dev page still routed in the production build with a dead API behind it (plus 5 dev routes, one destructive, and 9 more uncalled routes deleted); the server's refusal reasons never reaching the player because 19 places read the axios error shape and axios is not a dependency; the staff hire dialog missing its role and every staff card showing a blank age; nine routes answering 500 instead of 401 with no session, so an expired session never triggered the login redirect; and every real save logging an error-level line on boot for correct World Finals data — and four in the harness, each a check that could go red while the game was working. The live save was never opened; `C:/build/vbe` never written to | Gate 2 865/0 · Gate 3 three runs 865/0 with identical totals · Gate 5 90 routes, 0 errors, 0 5xx · Gate 6 18/18 on a copy of Rob's save · Gate 7 8/8 on a package repacked from the pushed commit. Full report: `docs/PRE-PUSH-CHECK.md` |
