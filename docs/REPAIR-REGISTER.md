...
### R-66 — CLOSED (15 Sep, b85ae80) MEDIUM: the installer offered a "Volley-Ball-Empire" install folder
...
| R-66 The installer's folder is always `<Programs>\Beach Volleyball Empire`. A remembered folder with another name (a test install, the pre-rename "Volley-Ball-Empire") is not offered, and the old version is still uninstalled from where it is. Release build 0.9.1 | 15 Sep, b85ae80 | a stale 0.9.0 install recreated with its own installer. Interactive: the directory page offers …\Programs\Beach Volleyball Empire, and Cancel restores the registry. Silent: lands there and empties the old folder (953 → 0). Reinstall reuses the folder; uninstall is clean; the live save hash is unchanged throughout. Installer 335,301,232 bytes, sha256 548f6206…; win-unpacked 952 files; no sidecars; no menu bar (captured) |
