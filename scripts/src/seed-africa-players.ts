...
/**
 * Upload a local file to GCS and return the DB imageUrl path.
 * DB path format: /objects/<entityId>
 * GCS path:       PRIVATE_OBJECT_DIR/<entityId>
 */
async function uploadPlayerImage(
  ...
  // PRIVATE_OBJECT_DIR format: /bucketName/prefix  or  /bucketName
  const parts = privateDir.startsWith("/") ? privateDir.slice(1) : privateDir;
  const slashIdx = parts.indexOf("/");
  const bucketName =
    slashIdx === -1 ? parts.replace(/\/$/, "") : parts.slice(0, slashIdx);
