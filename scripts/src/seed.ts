...
async function main() {
  ...
  const players = sqlite.prepare("SELECT id, name, is_draft_player FROM players").all() as Row[];
  ...
  for (const p of players) {
    ...
    if (p.is_draft_player) {
      ...
      sqlite.prepare("UPDATE players SET image_url = ?, age = ? WHERE id = ?").run(imageUrl, age, p.id);
    } else {
      sqlite.prepare("UPDATE players SET image_url = ? WHERE id = ?").run(imageUrl, p.id);
    }
  }
  console.log("Players done.");

  const staff = sqlite.prepare("SELECT id, name FROM staff").all() as Row[];
  console.log(`Updating ${staff.length} staff...`);
  for (const s of staff) {
    const imageUrl = getStaffImageUrl(s.name);
    sqlite.prepare("UPDATE staff SET image_url = ? WHERE id = ?").run(imageUrl, s.id);
    console.log(`  ${s.name} → ${imageUrl}`);
  }
  console.log("Staff done.");

  const locations = sqlite.prepare("SELECT id, city FROM locations").all() as Row[];
  console.log(`Updating ${locations.length} locations...`);
  for (const loc of locations) {
    sqlite.prepare("UPDATE locations SET image_url = ? WHERE id = ?").run(getLocationImageUrl(loc.city), loc.id);
  }
  ...
  const deals = sqlite.prepare("SELECT id, sponsor FROM promo_deals").all() as Row[];
  console.log(`Updating ${deals.length} promo deals...`);
  for (const d of deals) {
    const img = getSponsorImageUrl(d.sponsor);
    const stats = DEAL_STATS[d.sponsor];
    if (stats) {
      sqlite.prepare(
        "UPDATE promo_deals SET image_url = ?, amount = ?, requirement_wins = ? WHERE id = ?"
      ).run(img, stats.amount, stats.wins, d.id);
      console.log(`  ${d.sponsor} → $${stats.amount}, ${stats.wins} wins req`);
    } else {
      sqlite.prepare("UPDATE promo_deals SET image_url = ? WHERE id = ?").run(img, d.id);
    }
  }
